...
// TODO: See where the from should be here
// [13]
DatasetClause
...
ValuesClause
    : ('VALUES' InlineData)? -> $1 && { type: 'values', values: $2 }
    ;
...
// [30]
// TODO: Properly factor out [31]-[37]
Update1
    : Load
    | ClearOrDrop
    // | Clear
...
// [41]
Modify
    : WithClause? InsertDeleteClause UsingClause* 'WHERE' GroupGraphPattern -> { updateType: 'insertdelete', ...$1, ...$2, ...groupDatasets($4, 'using'), where: $6.patterns }
    ;
...
ArgList
    ...
    | '(' 'DISTINCT'? ( Expression ',' )* Expression ')' -> { type: 'functionCall', args: appendTo($4, $5), distinct: !!$3 }
    ;
...
// [107]
// TODO: See if we need a toVar here
VarOrIri
    : Var
    | iri
    ;
...
// SHOULD BE GOOD AFTER THIS POINT but need to double check - These are all the sparqlStar specific additions

// [174]
QuotedTP
    : '<<' qtSubjectOrObject Verb qtSubjectOrObject '>>' -> ensureSparqlStar(Parser.factory.quad($2, $3, $4))
    ;

// [175]
QuotedTriple
    : '<<'  DataValueTerm IriOrA DataValueTerm '>>' -> ensureSparqlStar(Parser.factory.quad($2, $3, $4))
    ;
...
ExprQuotedTP
    : '<<'  ExprVarOrTerm Verb ExprVarOrTerm '>>' -> ensureSparqlStar(Parser.factory.quad($2, $3, $4))
    ;
// [182]
