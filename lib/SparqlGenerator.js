...
Generator.prototype.toQuery = function (q) {
  ...
  if (q.variables){
    query += mapJoin(q.variables, undefined, function (variable) {
      return variable === "*" || isTerm(variable) ? this.toEntity(variable) :
...
Generator.prototype.toExpression = function (expr) {
  ...
  switch (expr.type.toLowerCase()) {
    ...
    case 'operation':
      ...
      case '*':
      case '/':
          return (args[0] === "*" || isTerm(args[0]) ? this.toEntity(args[0]) : '(' + this.toExpression(args[0]) + ')') +
                 ' ' + operator + ' ' +
                 (args[1] === "*" || isTerm(args[1]) ? this.toEntity(args[1]) : '(' + this.toExpression(args[1]) + ')');
      // Unary operators
      case '!':
...
Generator.prototype.toEntity = function (value) {
  ...
  if (isTerm(value)) {
    ...
    if (value.termType === "BlankNode") value = {termType: value.termType, value: value.value.replace(/^e_|^g_/, '')};
