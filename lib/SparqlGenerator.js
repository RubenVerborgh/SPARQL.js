...
function Generator(options, prefixes) {
  ...
  if (this._options.indention !== undefined) {
    INDENTION = this._options.indention
  }
}
...
Generator.prototype.toQuery = function (q) {
  ...
  if (q.variables)
    ...
    query += this.group(q.template, true) + NEWLINE;

  if (q.from)
    query += mapJoin(q.from.default || [], '', function (g) { return 'FROM ' + this.toEntity(g) + NEWLINE; }, this) +
             mapJoin(q.from.named || [], '', function (g) { return 'FROM NAMED ' + this.toEntity(g) + NEWLINE; }, this);
  if (q.where)
    query += 'WHERE ' + this.group(q.where, true)  + NEWLINE;

  if (q.updates)
    query += mapJoin(q.updates, ';' + NEWLINE, this.toUpdate, this);
  ...
  if (q.having)
    query += 'HAVING (' + mapJoin(q.having, undefined, this.toExpression, this) + ')' + NEWLINE;
  if (q.order)
  ...
  if (q.offset)
    query += 'OFFSET ' + q.offset + NEWLINE;
  if (q.limit)
    query += 'LIMIT ' + q.limit + NEWLINE;
...
Generator.prototype.baseAndPrefixes = function (q) {
  var base = q.base ? ('BASE <' + q.base + '>' + NEWLINE) : '';
  var prefixes = '';
  for (var key in q.prefixes) {
    if (this._options.allPrefixes || this._usedPrefixes[key])
      prefixes += 'PREFIX ' + key + ': <' + q.prefixes[key] + '>' + NEWLINE;
  }
...
Generator.prototype.array = function (items) {
  return mapJoin(items, NEWLINE, this.toPattern, this);
};
...
Generator.prototype.encodeTriples = function (triples) {
  ...
  for (var i = 0; i < triples.length; i++) {
    ...
    // Triple with same subject but different predicate
      ...
      predicate = triple.predicate;
      parts.push(';' + NEWLINE, INDENTATION, this.toEntity(predicate));
    }
...
Generator.prototype.group = function (group, inline) {
  ...
  return group.indexOf(NEWLINE) === -1 ? '{ ' + group + ' }' : '{' + NEWLINE + indent(group) + NEWLINE + '}';
};
...
Generator.prototype.union = function (union) {
  return mapJoin(union.patterns, NEWLINE + 'UNION' + NEWLINE, function (p) { return this.group(p, true); }, this);
};
...
Generator.prototype.values = function (valuesList) {
  ...
  // Create value rows
  return 'VALUES (' + keys.join(' ') + ') {' + NEWLINE +
    mapJoin(valuesList.values, NEWLINE, function (values) {
...
Generator.prototype.toUpdate = function (update) {
  ...
  case 'insertdelete':
    return (update.graph ? 'WITH ' + this.toEntity(update.graph) + NEWLINE : '') +
           (update.delete.length ? 'DELETE ' + this.group(update.delete, true) + NEWLINE : '') +
           (update.insert.length ? 'INSERT ' + this.group(update.insert, true) + NEWLINE : '') +
