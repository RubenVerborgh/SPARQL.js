...
function Generator(options, prefixes) {
  ...
  if (this._options.newline === undefined) {
    this._options.newline = NEWLINE;
  }
  if (this._options.indentation === undefined) {
    this._options.indentation = INDENTATION;
  }
}
...
Generator.prototype.toQuery = function (q) {
  ...
  if (q.variables)
    ...
    query += this.group(q.template, true) + this._options.newline;

  if (q.from)
    query += mapJoin(q.from.default || [], '', function (g) { return 'FROM ' + this.toEntity(g) + this._options.newline; }, this) +
             mapJoin(q.from.named || [], '', function (g) { return 'FROM NAMED ' + this.toEntity(g) + this._options.newline; }, this);
  if (q.where)
    query += 'WHERE ' + this.group(q.where, true) + this._options.newline;

  if (q.updates)
    query += mapJoin(q.updates, ';' + this._options.newline, this.toUpdate, this);
  ...
  if (q.group)
    ...
    }, this) + this._options.newline;
  if (q.having)
    query += 'HAVING (' + mapJoin(q.having, undefined, this.toExpression, this) + ')' + this._options.newline;
  if (q.order)
  ...
  if (q.offset)
    query += 'OFFSET ' + q.offset + this._options.newline;
  if (q.limit)
    query += 'LIMIT ' + q.limit + this._options.newline;
...
Generator.prototype.baseAndPrefixes = function (q) {
  var base = q.base ? ('BASE <' + q.base + '>' + this._options.newline) : '';
  var prefixes = '';
  for (var key in q.prefixes) {
    if (this._options.allPrefixes || this._usedPrefixes[key])
      prefixes += 'PREFIX ' + key + ': <' + q.prefixes[key] + '>' + this._options.newline;
  }
...
Generator.prototype.array = function (items) {
  return mapJoin(items, this._options.newline, this.toPattern, this);
};
...
Generator.prototype.encodeTriples = function (triples) {
  ...
  for (var i = 0; i < triples.length; i++) {
    ...
    if (triple.subject !== subject) {
      ...
      if (subject)
        parts.push('.' + this._options.newline);
    ...
    // Triple with same subject but different predicate
      ...
      predicate = triple.predicate;
      parts.push(';' + this._options.newline, this._options.indentation, this.toEntity(predicate));
    }
...
Generator.prototype.group = function (group, inline) {
  ...
  return group.indexOf(this._options.newline) === -1 ? '{ ' + group + ' }' : '{' + this._options.newline + this.indent(group) + this._options.newline + '}';
};
...
Generator.prototype.union = function (union) {
  return mapJoin(union.patterns, this._options.newline + 'UNION' + this._options.newline, function (p) { return this.group(p, true); }, this);
};
...
Generator.prototype.values = function (valuesList) {
  ...
  // Create value rows
  return 'VALUES (' + keys.join(' ') + ') {' + this._options.newline +
    mapJoin(valuesList.values, this._options.newline, function (values) {
...
Generator.prototype.toUpdate = function (update) {
  ...
  case 'insertdelete':
    return (update.graph ? 'WITH ' + this.toEntity(update.graph) + this._options.newline : '') +
           (update.delete.length ? 'DELETE ' + this.group(update.delete, true) + this._options.newline : '') +
           (update.insert.length ? 'INSERT ' + this.group(update.insert, true) + this._options.newline : '') +
...
Generator.prototype.indent = function(text) { return text.replace(/^/gm, this._options.indentation); }
