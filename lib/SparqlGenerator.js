...
function Generator(options, prefixes) {
  ...
  var iriList = prefixesArray
    ...
    .join('|')
    .replace(/[\]\/\(\)\*\+\?\.\\\$]/g, '\\$&');
  this._prefixRegex = new RegExp('^(' + iriList + ')[a-zA-Z][\\-_a-zA-Z0-9]*$', 'g');
  this._prefixByIri = prefixesArray.reduce(function (acc, prefix) {
  ...
  for (var key in PATTERNS)
    this._patterns[key] = PATTERNS[key].bind(this);
}
...
Generator.prototype.toQuery = function (q) {
  ...
  if (q.variables)
    ...
    query += this._patterns.group(q.template, true) + '\n';
  ...
  if (q.where)
    query += 'WHERE ' + this._patterns.group(q.where, true)  + '\n';
...
Generator.prototype.toPattern = function (pattern) {
  ...
  if (!(type in this._patterns))
    throw new Error('Unknown entry type: ' + type);
  return this._patterns[type](pattern);
}
var PATTERNS = {
  triple: function (t) {
  ...
  bgp: function (bgp) {
    return mapJoin(bgp.triples, '\n', this._patterns.triple, this);
  },
  graph: function (graph) {
    return 'GRAPH ' + this.toEntity(graph.name) + ' ' + this._patterns.group(graph);
  },
  group: function (group, inline) {
    group = inline !== true ? this._patterns.array(group.patterns || group.triples)
  ...
  optional: function (optional) {
    return 'OPTIONAL ' + this._patterns.group(optional);
  },
  union: function (union) {
    return mapJoin(union.patterns, '\nUNION\n', function (p) { return this._patterns.group(p, true); }, this);
  },
  minus: function (minus) {
    return 'MINUS ' + this._patterns.group(minus);
  },
  ...
  switch (expr.type.toLowerCase()) {
    ...
    case 'operation':
      ...
      case 'notexists':
        operator = 'NOT EXISTS';
      case 'exists':
        return operator + ' ' + this._patterns.group(args[0], true);
      // Other expressions
      default:
...
Generator.prototype.encodeIRI = function (iri) {
  ...
  this._prefixRegex.lastIndex = 0;
  var prefixMatch, longestPrefix, prefixLength = -1;
  while (prefixMatch = this._prefixRegex.exec(iri)) {
    var matchedPrefixIRI = prefixMatch[1];
    if (matchedPrefixIRI.length > prefixLength) {
      longestPrefix = this._prefixByIri[matchedPrefixIRI];
      prefixLength = matchedPrefixIRI.length;
    }
  }

  if (longestPrefix !== undefined) {
    this._usedPrefixes[longestPrefix] = true;
...
Generator.prototype.toUpdate = function (update) {
  ...
  case 'insert':
    return 'INSERT DATA '  + this._patterns.group(update.insert, true);
  case 'delete':
    return 'DELETE DATA '  + this._patterns.group(update.delete, true);
  case 'deletewhere':
    return 'DELETE WHERE ' + this._patterns.group(update.delete, true);
  case 'insertdelete':
    return (update.graph ? 'WITH ' + this.toEntity(update.graph) + '\n' : '') +
           (update.delete.length ? 'DELETE ' + this._patterns.group(update.delete, true) + '\n' : '') +
           (update.insert.length ? 'INSERT ' + this._patterns.group(update.insert, true) + '\n' : '') +
           'WHERE ' + this._patterns.group(update.where, true);
  case 'add':
  case 'copy':
  case 'move':
