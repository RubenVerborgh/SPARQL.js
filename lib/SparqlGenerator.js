...
Generator.prototype.encodeTriples = function (triples) {
  var parts = [];
  var sameSubject = false;
  var samePredicate = false;

  for (var i = 0; i < triples.length; i++) {
    var t = triples[i];
    if (!sameSubject) {
      parts.push(this.toEntity(t.subject), ' ');
    }
    if (!(sameSubject && samePredicate)) {
      parts.push(this.toEntity(t.predicate), ' ');
    }
    parts.push(this.toEntity(t.object));

    var next = triples[i + 1];
    if (next) {
      sameSubject = t.subject === next.subject;
      samePredicate = t.predicate === next.predicate;
      parts.push(
        sameSubject && samePredicate ? ', ' :
        sameSubject ? (';\n' + INDENTATION) :
        '.\n'
      );
    } else {
