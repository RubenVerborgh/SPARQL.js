...
var termToString = require("rdf-string").termToString;
...
Generator.prototype.toQuery = function (q) {
  ...
  if (q.variables){
    query += mapJoin(q.variables, undefined, function (variable) {
      return isString(variable) || isTerm(variable) ? this.toEntity(variable) :
             '(' + this.toExpression(variable.expression) + ' AS ' + "?" + variable.variable.value + ')';
    }, this) + ' ';
  }
  else if (q.template)
  ...
  if (q.group)
    query += 'GROUP BY ' + mapJoin(q.group, undefined, function (it) {
      ...
      return it.variable ? '(' + result + ' AS ' + "?" + it.variable.value + ')' : result;
    }, this) + this._newline;
  if (q.having)
...
Generator.prototype.bind = function (bind) {
  return 'BIND(' + this.toExpression(bind.expression) + ' AS ' + "?" + bind.variable.value + ')';
};
...
Generator.prototype.toExpression = function (expr) {
  ...
  switch (expr.type.toLowerCase()) {
    ...
    case 'operation':
      ...
      case '*':
      case '/':
          return (isString(args[0]) || isTerm(args[0]) ? this.toEntity(args[0]) : '(' + this.toExpression(args[0]) + ')') +
                 ' ' + operator + ' ' +
                 (isString(args[1]) || isTerm(args[1]) ? this.toEntity(args[1]) : '(' + this.toExpression(args[1]) + ')');
      // Unary operators
      case '!':
...
Generator.prototype.toEntity = function (value) {
  ...
  if (isTerm(value)) {
    // Remove the prefix but with taking a copy to avoid altering the input
    if (value.termType === "BlankNode") value = {termType: value.termType, value: value.value.substr(2)};
...
function isTerm(object) { return object.termType !== undefined; }
